...
export class CategoryService {
  ...
  getCategoriesWithSubcategories(): Observable<Category[]> {
    return this.http.get<Category[]>(`${this.apiUrl}/categories?include=subcategories`);
  }
  getPopularCoursesBySubcategory(subcategoryId: number): Observable<Course[]> {
    return this.http.get<Course[]>(`${this.apiUrl}/subcategories/${subcategoryId}/courses?sort=popularity&limit=5`);
  }
